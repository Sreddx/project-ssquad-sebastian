...
class AppTestCase(unittest.TestCase):
    ...
    def test_timeline_post_endpoint(self):
        response=self.client.post("/api/newPost", data= {"name":"alice","email":"alicia@example.com","content":"Hello world,I'm Alicia!"})
        assert response.is_json
        json = response.get_json()
    ...
    def test_malformed_timeline_post(self):
        ...
        response = self.client.post("/api/newPost", data={"email": "john@example.com", "content": "Hello world, I'm John!"})
        assert response.status_code == 400
        html = response.get_data(as_text=True)
        assert "Name is required." in html

        # POST request with empty content
        response = self.client.post("/api/newPost", data={"name": "John Doe", "email": "john@example.com", "content": ""})
        assert response.status_code == 400
        html = response.get_data(as_text=True)
        assert "Content is required." in html

        # POST request with malformed email
        response = self.client.post("/api/newPost", data={"name": "John Doe", "email": "not-an-email", "content": "Hello  world, I'm John"})
        assert response.status_code == 400
        html = response.get_data(as_text=True)
